# BizStack — Outreach Emails (Ready to Send)

> **Send order:** SBDC **first** → LISC → VCC → VSBFA.
> Every email goes from **hello@bizstackperks.com**, same numbers every time.
> Attach the one-page pitch PDF on the Day 0 email to lenders.
> Send emails on the same day across lenders (Day 0), then run texts/emails on the same deadlines.

## ⚠️ BEFORE SENDING — get these ready (see `docs/CHECKLIST.md`)
- [ ] Bank statement showing the $10K equity (current, last 30 days) — `~/Documents/statement_equity_FINAL.pdf` ✅ exists
- [x] One-page pitch exported to PDF (`docs/SBA_7a_LOAN_PITCH.pdf`) — **generated Sept 18, 2026**
- [ ] Invoices + signed receipts from both cash jobs (if not yet signed, send tomorrow AM — do NOT block the SBDC email on this)
- [x] Fill `[First Name]` + `[Lender Name]` fields below — **verified Sep 18, 2026**

---

## STEP 0 — EMAIL THE SBDC FIRST (do this before anything else)

**To:** sbdc@hrchamber.com · **Williamsburg rep:** jspiers@hrchamber.com (Jolie Spiers, Williamsburg Community Building) · **Cc:** hello@bizstackperks.com
**Subject:** Free advising request — startup, $50K microloan prep (STR services + construction)
**Phone:** (757) 664-2592

```
Hi Hampton Roads SBDC team,

I'm Shaun O'Leary, owner of BizStack (bizstackperks.com). We run two linked
service lines for the short-term-rental market: Buildstack Construction, a
licensed GC doing renovations and STR make-readies, and Broom Service, turnover
cleaning and co-hosting for Airbnb/Vrbo properties — across Williamsburg–Hampton
Roads and Currituck County, NC. I have 25 years in the construction trade.

I'm raising a $50,000 microloan for working capital, equipment, and lead
generation, and I'd like help before I apply — specifically to tighten my
12-month projections and package for a microloan with LISC Hampton Roads,
Virginia Community Capital, and VSBFA. Could I set up a free advising
appointment at your earliest convenience?

Quick background:
- New entity, 1 year formally operating; 25 years of industry experience
- Real 2026 revenue of $32,516: two completed construction projects ($31,900)
  plus 3 turnovers and a 15% co-host commission across 3 host relationships
- The business already runs on software I built: instant online quotes, a 24/7
  AI phone/SMS assistant, GPS crew time-tracking, Stripe payments, and payroll
- $10,000 of my own capital going in
- Target: repayment covered inside year one (DSCR above the SBA's 1.10 floor)

I can send my one-page summary, license, insurance, and financials ahead of the
appointment. What days work for you?

Thanks,
Shaun O'Leary
BizStack
252-665-5891 · hello@bizstackperks.com
bizstackperks.com
```

---

## DAY 0 — EMAIL to LISC HAMPTON ROADS

**To:** smallbusiness@lisc.org (LISC small business lending intake) · **Cc:** wmartin@lisc.org (Will Martin, Capital Access & Deployment Officer, LISC Virginia) · **Cc:** hello@bizstackperks.com
**Subject:** $50K microloan — BizStack (STR construction + cleaning/co-hosting), Hampton Roads
**Attach:** SBA_7a_LOAN_PITCH.pdf

```
Hi [First Name],

I'm Shaun O'Leary, owner of BizStack (bizstackperks.com). We operate two linked
service lines for the short-term-rental market:
- Buildstack Construction — licensed general contractor: renovations, kitchens &
  baths, roofing & siding, decks, and STR make-readies
- Broom Service — STR turnover cleaning and co-hosting for Airbnb/Vrbo properties
Service area: Williamsburg–Hampton Roads, VA and Currituck/Elizabeth City, NC.

I'm applying for a $50,000 SBA microloan and would like to work with LISC
Hampton Roads.

Quick facts:
- 25 years in the construction trade; I'm the operator, not an investor
- $32,516 in 2026 revenue: two completed construction projects ($31,900) and
  cleaning + co-host commission across 3 host relationships ($616.50)
- The business already runs on software I built: instant online quotes, a 24/7
  AI phone/SMS assistant, GPS crew time-tracking, Stripe payments, and payroll
- $10,000 of my own capital going in
- 12-month projections show the ~$930/month payment covered inside year one

Use of funds: $20K working capital/payroll & supplies, $18K equipment/tools,
$12K marketing & lead generation.

My full package is ready — one-page summary attached. What's the best next step
to submit? I can send documents same-day by email.

Thanks,
Shaun O'Leary
BizStack
252-665-5891 · hello@bizstackperks.com
bizstackperks.com
```

---

## DAY 0 — EMAIL to VIRGINIA COMMUNITY CAPITAL (VCC)

**To:** jbarnes@vccva.org (Joey Barnes, Small Business Lending Manager) · **Cc:** hello@bizstackperks.com · Phone (757) 962-0955
**Subject:** $50K microloan — BizStack (STR construction + cleaning/co-hosting), Hampton Roads
**Attach:** SBA_7a_LOAN_PITCH.pdf

```
Hi [First Name],

I'm Shaun O'Leary, owner of BizStack (bizstackperks.com). We operate two linked
service lines for the short-term-rental market:
- Buildstack Construction — licensed general contractor: renovations, kitchens &
  baths, roofing & siding, decks, and STR make-readies
- Broom Service — STR turnover cleaning and co-hosting for Airbnb/Vrbo properties
Service area: Williamsburg–Hampton Roads, VA and Currituck/Elizabeth City, NC.

I'm applying for a $50,000 SBA microloan and would like to work with Virginia
Community Capital.

Quick facts:
- 25 years in the construction trade; I'm the operator, not an investor
- $32,516 in 2026 revenue: two completed construction projects ($31,900) and
  cleaning + co-host commission across 3 host relationships ($616.50)
- The business already runs on software I built: instant online quotes, a 24/7
  AI phone/SMS assistant, GPS crew time-tracking, Stripe payments, and payroll
- $10,000 of my own capital going in
- 12-month projections show the ~$930/month payment covered inside year one
- No judgments, no liens, no bankruptcies

Use of funds: $20K working capital/payroll & supplies, $18K equipment/tools,
$12K marketing & lead generation.

My full package is ready — one-page summary attached. What's the best next step
to submit? I can send documents same-day by email.

Thanks,
Shaun O'Leary
BizStack
252-665-5891 · hello@bizstackperks.com
bizstackperks.com
```

---

## DAY 0 — EMAIL to VSBFA (Virginia Small Business Financing Authority)

**To:** VSBFA@sbsd.virginia.gov
**Phone:** (804) 786-1049 — call first, ask for a **Regional Lending Manager** (Linda K. Tackett / Terrance Rogers), then email them directly
**Subject:** $50K microloan — BizStack (STR construction + cleaning/co-hosting), Hampton Roads
**Attach:** SBA_7a_LOAN_PITCH.pdf

```
Hi [First Name],

I'm Shaun O'Leary, owner of BizStack (bizstackperks.com) in Williamsburg, VA.
We run two linked service lines for the short-term-rental market:
- Buildstack Construction — licensed general contractor: renovations and STR
  make-readies
- Broom Service — STR turnover cleaning and co-hosting for Airbnb/Vrbo properties

I'm applying for a VSBFA microloan ($50K) for working capital, equipment, and
lead generation.

Quick facts:
- 25 years in the construction trade; I'm the operator, not an investor
- $32,516 in 2026 revenue: two completed construction projects ($31,900) and
  cleaning + co-host commission across 3 host relationships ($616.50)
- $10,000 of my own capital going in
- 12-month projections show the ~$930/month payment covered inside year one

Use of funds: $20K working capital, $18K equipment/tools, $12K marketing &
lead generation. (These are my own working-capital, equipment, and software
needs — not third-party construction financing.)

I understand you prefer to talk first — happy to call at your convenience for a
short intro. My one-page summary is attached; my full package is ready same-day
by email.

Thanks,
Shaun O'Leary
BizStack
252-665-5891 · hello@bizstackperks.com
bizstackperks.com
```

---

## DAY 1 — TEXT (to any lender with a mobile #, business hours 9–5)

```
Hi [First Name], this is Shaun O'Leary with BizStack in Williamsburg. I emailed
yesterday about a $50K microloan for my company (STR construction + cleaning/
co-hosting). Wanted to make sure it landed — happy to send everything by email
today. Thanks — 252-665-5891 · bizstackperks.com
```

---

## DAY 3 — EMAIL 2 (add value, no begging) — to all lenders

**Subject:** Re: $50K microloan — BizStack
**Attach:** 12-month projections (when ready)

```
Hi [First Name],

Following up with one thing I didn't include yesterday: the reason I think this
is a strong file despite being a new entity is that the hard part already
exists — the operating system. Quotes generate from property data in minutes,
every call and text is answered 24/7 by AI, crews clock in with GPS and upload
photo-verified work, and payments and payroll run through Stripe.

And it's two revenue lines on one platform: guest-funded cleaning fees collected
at booking (pre-paid, predictable) plus fixed-price construction scopes. Loan
dollars go straight into jobs and turnover volume — not into figuring out how to
run the office.

I've attached my 12-month projections. Happy to do a short phone call at
whatever time suits you. What else would you like to see?

Thanks,
Shaun O'Leary
BizStack · 252-665-5891 · hello@bizstackperks.com
```

---

## DAY 7 — TEXT 2 (soft check-in)

```
Hi [First Name], Shaun with BizStack. Just checking in on the $50K microloan
request — is there anything you need from me to move it forward? I can get
documents over same-day. Thanks — 252-665-5891 · bizstackperks.com
```

---

## DAY 14 — EMAIL 3 (close the loop, keep the door open)

**Subject:** Re: $50K microloan — checking in

```
Hi [First Name],

I know you're busy, so I'll leave it here: my $50K microloan request and complete
package are ready whenever you are. If this isn't the right fit at [Lender Name],
could you point me to who handles startups in your shop — or the SBDC — so I
don't bother you further?

Either way, thank you for your time.

Shaun O'Leary
BizStack · 252-665-5891
hello@bizstackperks.com · bizstackperks.com
```

**Then stop. Move to the next lender. Revisit in 60 days.**

---

## REPLY PLAYBOOK (paste these back)

**They say "send documents":**
> Great — thank you. Sending now from hello@bizstackperks.com. If anything is
> missing, just text me here and I'll have it to you within the hour. — Shaun,
> 252-665-5891

**They ask about credit (600s):**
> Happy to address it. No judgments, no liens, no bankruptcies. I'm actively
> paying down utilization now — the score is on the way up, and the file's
> strength is 25 years of trade experience, two revenue lines, and a projected
> DSCR well above the 1.10 requirement. — Shaun

**They say "we need revenue history":**
> Understood — that's exactly why I'm applying for a microloan rather than a
> bank term loan. The SBA's 2026 small-loan rules allow projected cash flow for
> startups, and my projections show the payment covered in year one. Would the
> SBDC-prepared package help you take a second look? — Shaun

**They say yes / approve:**
> Thank you — I appreciate it. Send me the next steps and I'll return everything
> same-day. — Shaun, 252-665-5891

**They decline:**
> I appreciate you looking at it. Two asks: who else should I talk to, and what
> one thing would change your answer? Thank you — Shaun

---

## SENDING LOG (live)
- [x] **SENT Sept 18, 2026** — SBDC (Step 0): `sbdc@hrchamber.com` cc `hello@bizstackperks.com`,
      subject "Free advising request — startup, $50K microloan prep". Verified delivered via
      mail.privateemail.com (465). **Next:** LISC → VCC → VSBFA Day-0 emails (pitch PDF attached).

## SENDING ORDER CHECKLIST
- [x] Send SBDC email (Step 0) — **sent Sept 18, 2026**
- [x] Run `pitch_numbers.py` → verify numbers still match, update pitch if needed
- [x] Export pitch PDF (`docs/SBA_7a_LOAN_PITCH.pdf`) — generated Sept 18, 2026
- [ ] Pull bank statement for $10K equity
- [ ] Day 0 send all three lender emails (LISC, VCC, VSBFA)
- [ ] Print/sign invoices + receipts with homeowners, add to package
- [ ] Collect insurance scope paperwork (Sunnywood)
- [ ] Same-day response to all replies, same numbers every touch